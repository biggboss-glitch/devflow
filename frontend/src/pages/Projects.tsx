...
export const Projects: React.FC = () => {
  ...
  const handleViewProject = (project: Project) => {
    setProjectModal({ isOpen: true, type: 'view', project });
  };
  ...
  const handleEditProject = (project: Project) => {
    ...
    setProjectModal({ isOpen: true, type: 'edit', project });
  };

  const handleSaveEdit = async () => {
    if (!projectModal.project || !formData.name.trim()) {
      setError('Project name is required');
      return;
    }
    ...
    try {
      setLoading(true);
      await apiClient.patch(`/projects/${projectModal.project.id}`, formData);
      setProjects(projects.map(p => p.id === projectModal.project!.id ? { ...p, ...formData } : p));
      setProjectModal({ isOpen: false, type: 'view', project: null });
      setFormData({ name: '', description: '', status: 'active', team_id: '' });
      setError('');
    } catch (err: any) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <h2 className="text-2xl font-bold mb-4">
              {projectModal.type === 'edit' ? 'Edit Project' : 'Project Details'}
            </h2>
            ...
            {projectModal.type === 'edit' ? (
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="font-medium">{projectModal.project.name}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">{getTeamName(projectModal.project.team_id || '')}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Description</p>
                  <p className="font-medium">{projectModal.project.description || 'No description'}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-600">Created</p>
                  <p className="font-medium">{new Date(projectModal.project.created_at).toLocaleDateString()}</p>
                </div>
                <button
                  onClick={closeModal}
